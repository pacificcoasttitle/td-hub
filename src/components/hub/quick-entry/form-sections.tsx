    ...
    <div>
      ...
      {isRefiLike && (
        <div className="border-t border-gray-200 pt-4 mt-4">
          ...
          {!s.hasSecBorrower ? (
            <button type="button" onClick={() => s.setHasSecBorrower(true)} className="text-xs font-medium text-[#F26B2B] flex items-center gap-1 min-h-[36px]">+ Add secondary borrower</button>
          ) : (
            <>
              <div className="flex items-center justify-between mt-2"><span className="text-xs text-[#6B7280]">Secondary borrower</span><button type="button" onClick={() => { s.setHasSecBorrower(false); s.setSecBorrower({ ...EP }); }} className="text-xs text-red-500 min-h-[36px]">Remove</button></div>
          ...
          {s.borrowerIsOrg && (
            <select value={s.borrowerOrgType} onChange={e => s.setBorrowerOrgType(e.target.value)} className="mt-2 h-9 px-2 border border-gray-200 rounded-lg text-xs focus:outline-none focus:border-[#F26B2B]">
              <option value="">Type…</option>
              {ORG_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
            </select>
          )}
