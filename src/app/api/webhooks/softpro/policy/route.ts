...
export async function POST(req: NextRequest) {
  ...
  if (!auth.ok) {
    logSoftProWebhookRejection('/api/webhooks/softpro/policy', auth.reason);
    await logWebhook(requestId, startedAt, 'webhook_policy_auth_rejected', null, false, auth.reason);
  ...
  await logWebhook(requestId, startedAt, 'webhook_policy_received', rawBody);

  const parsed = policyPayloadSchema.safeParse(rawBody);
  if (!parsed.success) {
    await logWebhook(requestId, startedAt, 'webhook_policy_invalid', rawBody, false, parsed.error.message);
    return NextResponse.json({ success: false, error: 'Invalid payload', details: parsed.error.issues });
  }

  try {
    const result = await handlePolicyWebhook(parsed.data);
    await logWebhook(requestId, startedAt, 'webhook_policy_processed', rawBody, result.success, undefined, result);
    return NextResponse.json(result);
  } catch (err) {
    const msg = err instanceof Error ? err.message : 'Unknown error';
    await logWebhook(requestId, startedAt, 'webhook_policy_error', rawBody, false, msg);
