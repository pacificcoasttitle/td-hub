...
export async function loadConfirmationData(fileNumber: string) {
  ...
  const tpData = await loadTitlePointData(row.id);
  ...
  async function loadTitlePointData(orderId: number) {
    ...
    return {
      ...
      documents: {
        lv: { status: lvRow?.status ?? 'not_started', s3Url: docUrls.legal_vesting },
        grantDeed: { status: gdRow?.status ?? 'not_started', s3Url: docUrls.grant_deed },
        tax: { status: taxRow?.status ?? 'not_started', s3Url: docUrls.tax },
      },
    };
  }
