...
export function ContactsTab() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/contacts?${apiParams}`, { signal: controller.signal })
      .then((res) => { if (!res.ok) throw new Error(`Failed to load contacts (${res.status})`); return res.json() as Promise<PaginatedContacts>; })
  ...
  function handleSearchChange(value: string) {
    ...
    debounceRef.current = setTimeout(() => { router.push(buildUrl({ cSearch: value, cPage: 1 })); }, 300);
  }
  ...
  return (
    <>
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        <SearchInput value={searchInput} onChange={handleSearchChange} placeholder="Search name, email, company…" />
        <select value={currentRole} onChange={(e) => router.push(buildUrl({ role: e.target.value, cPage: 1 }))}
        ...
        <ActiveToggle value={currentActive} onChange={(v) => router.push(buildUrl({ active: v, cPage: 1 }))} />
        {data && !loading && <span className="text-sm text-[#6B7280] ml-auto">{data.total} contact{data.total !== 1 ? 's' : ''}</span>}
      </div>
              ...
              <tbody className="divide-y divide-gray-100">
                {loading
                  ...
                  : data && data.contacts.length > 0
                    ? data.contacts.map((c) => (
                        <ContactRow key={c.id} contact={c} expanded={expandedId === c.id}
                          onToggle={() => setExpandedId(expandedId === c.id ? null : c.id)} />
                      ))
                    : null}
              </tbody>
            </table>
            {!loading && data && data.contacts.length === 0 && <EmptyState message="No contacts found. Try adjusting your search or filters." />}
          </div>
        )}
  ...
  return (
    <>
      <tr onClick={onToggle} className="hover:bg-gray-50 cursor-pointer transition-colors">
        ...
        <td className="px-4 py-3 whitespace-nowrap"><SourceBadge source={contact.sourceSystem} /></td>
        <td className="px-4 py-3 whitespace-nowrap"><StatusDot active={contact.isActive} /></td>
      </tr>
      {expanded && (
...
export function ActiveToggle({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  const options = [{ value: 'true', label: 'Active' }, { value: 'false', label: 'Inactive' }, { value: 'all', label: 'All' }];
  return (
...
function RoleBadge({ role }: { role: string }) {
  ...
  return <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium capitalize ${color}`}>{role.replace(/_/g, ' ')}</span>;
}

function SourceBadge({ source }: { source: string | null }) {
  const isSoftPro = source === 'softpro';
  return (
    <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${isSoftPro ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-600'}`}>
