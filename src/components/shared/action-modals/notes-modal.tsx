      ...
      <div className="p-5 space-y-4">
        <div>
          <textarea value={newNote} onChange={(e) => setNewNote(e.target.value)} placeholder="Add a note…" rows={3}
            className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm resize-none outline-none focus:ring-2 focus:ring-gray-200" />
          {error && <p className="text-xs text-red-600 mt-1">{error}</p>}
          <button onClick={addNote} disabled={!newNote.trim() || saving}
            className="mt-2 px-4 h-9 text-white text-sm font-medium rounded-lg disabled:opacity-50 transition-colors"
