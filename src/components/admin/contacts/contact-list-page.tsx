          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {loading ? Array.from({ length: 8 }).map((_, i) => (
                  <tr key={i}>{Array.from({ length: colCount + 1 }).map((_, j) => (
                  ...
                  return (
                    <tr key={c.id} className="hover:bg-gray-50 transition-colors group">
                      <td className="px-4 py-3 font-medium text-[#1A1A2E] whitespace-nowrap cursor-pointer" onClick={() => setExpandedId(expandedId === c.id ? null : c.id)}>{name}</td>
            ...
            {!loading && contacts.length === 0 && (
              <div className="p-12 text-center">
                ...
                <p className="text-sm text-[#6B7280] mt-1">Try adjusting your search or filters, or add a new contact.</p>
              </div>
            )}
          </div>
        )}
